...
async function renderAdminRanking() {
  ...
  _adminRanking = participantes.map(p => {
    ...
    let pts = 0, exactos = 0, correctos = 0, fallos = 0, gol = p.favorito || null, done = 0, r32 = 0, finales = 0, campeon = null;
    if (q) {
      ...
      // Campeón = ganador de la Final (llave 104) en el bracket del jugador
      const fin = brac[104];
      if (fin && fin.gl !== undefined && fin.gv !== undefined) {
        campeon = fin.gl > fin.gv ? fin.l : fin.gv > fin.gl ? fin.v : (fin.penales || null);
      }
    }
    return { id: p.id, nombre: p.nombre || '', alias: p.alias || '', email: p.email || '', codigo: p.codigo || '', oculto: !!p.oculto, gol, campeon, pts, done, r32, finales, exactos, correctos, fallos };
  ...
  c.innerHTML = `<p style="font-size:11px;color:var(--muted);margin-bottom:.5rem">Haz clic en un jugador para ver sus predicciones y puntos por partido. <b>Exactos</b> = marcador exacto · <b>Result.</b> = acertó ganador/empate.</p>
    ...
    <thead><tr style="border-bottom:2px solid var(--borde)">
      ${['#', 'Nombre', 'Alias', 'Correo', 'Goleador', 'Campeón', 'Pred.', 'R32', 'QF-F', 'Exactos', 'Result.', 'Puntos'].map(h => `<th style="text-align:left;padding:7px;color:var(--muted);font-size:10px;text-transform:uppercase">${h}</th>`).join('')}
    </tr></thead><tbody>
    ${_adminRanking.map((r, i) => `
      <tr style="border-bottom:1px solid rgba(0,0,0,0.05);cursor:pointer;${r.oculto ? 'opacity:.55' : ''}" onclick="verPerfil('${r.id}')" title="Ver predicciones de ${(r.alias||r.nombre||'').replace(/'/g,'')}">
        ...
        <td style="padding:7px;font-weight:600">${r.campeon ? flagBadge(r.campeon, 14) + ' ' + r.campeon : '—'}</td>
...
function exportarRankingCSV() {
  ...
  const cols = ['Pos', 'Nombre', 'Alias', 'Correo', 'Codigo', 'Pais goleador', 'Campeon', 'Predichos', 'R32 (de 16)', 'Cuartos-Final (de 7)', 'Exactos', 'Correctos', 'Fallos', 'Puntos', 'Oculto'];
  const rows = _adminRanking.map((r, i) => [i + 1, r.nombre, r.alias, r.email, r.codigo, r.gol || '', r.campeon || '', r.done, r.r32, r.finales, r.exactos, r.correctos, r.fallos, r.pts, r.oculto ? 'si' : 'no']);
